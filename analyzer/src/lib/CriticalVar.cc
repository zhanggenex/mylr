...
void CriticalVarPass::markEdge(CFGEdge &CE, 
		ErrnoFlag flag, EdgeErrnoFlag &errnoEdges) {
	errnoEdges[CE] = flag;
}

/// Mark the edge from predBB to succBB with errno flag.
void CriticalVarPass::markEdgeBlockToBlock(BasicBlock *predBB, 
		BasicBlock *succBB,
		ErrnoFlag flag, EdgeErrnoFlag &errnoEdges) {
	TerminatorInst *TI;
...
void CriticalVarPass::markAllEdgesFromBlock(
		BasicBlock *BB, ErrnoFlag flag,
...
void CriticalVarPass::markAllEdgesToBlock(
		BasicBlock *BB, ErrnoFlag flag,
		EdgeErrnoFlag &errnoEdges) {
...
void CriticalVarPass::recurCheckValueErrno(
		Function *F, std::list<EdgeValue> &EV,
    std::set<Value *> &PV, 
		EdgeErrnoFlag &errnoEdges) {
...
void CriticalVarPass::checkValueErrno(Function *F, 
		Value *V, EdgeErrnoFlag &errnoEdges) {
	std::list<EdgeValue> EV;
	std::set<Value *> PV;
...
void CriticalVarPass::checkReturnValue(Function *F, 
...
void CriticalVarPass::findSecurityChecks(
		Function *F,
    EdgeErrnoFlag &errnoEdges,
...
void CriticalVarPass::findCriticalVariable(
...
void CriticalVarPass::identifyCriticalVariables(
		Function *F,
    std::set<Value *> &securityChecks,
		std::map<Value *, std::set<Value *>> &CheckToVars) {
...
void CriticalVarPass::getAliasPointers(
		Value *Addr,
...
bool CriticalVarPass::possibleUseStResult(
		Instruction *Inst,
		Instruction *St) {
...
void CriticalVarPass::trackAliasAddrAfterModification(
		Value *Addr,
    std::set<Value *> &pSet,
    Instruction *StInst,
...
bool CriticalVarPass::isExploitable(
		Value *V, Value *Addr,
...
void CriticalVarPass::findUseDefFromCVAddr(
		Function *F,
    Value *cvAddr,
    std::set<Value *> &pSet,
    std::set<InstructionUseDef> &udSet,
...
/// 2. Stack variable???
void CriticalVarPass::findUseDefOfCriticalVar(
		Value *CV,
    Value *cvAddr,
    std::set<Value *> &pSet,
    std::set<InstructionUseDef> &udSet,
...
///
void CriticalVarPass::filterDefFromCheckedValue(
		Value *CV,
    std::set<InstructionUseDef> &udSet,
		Value *SeCheck) {
...
void CriticalVarPass::trackCalledFunc(
		Function *F, 
		Value *SrcAddr,
...
Value *CriticalVarPass::trackSrcOfVal(
		Function *F, 
		Value *V,
...
void CriticalVarPass::findSrcForModification(
		Function *F,
    Instruction *Inst,
    std::list<Instruction *> &ContextInfo,
		std::map<Instruction *, std::list<Instruction *>> &TrackResults) {
